	...
	}{
		{
			txs: []*types.Tx{
				types.NewTx(types.TxData{
					...
					Outputs: []*types.TxOutput{types.NewTxOutput(*consensus.EYAssetID, 5000, nil)},
				}),
			},
			amount: 5000,
			err:    nil,
			...
			block: &bc.Block{
				...
				Transactions: []*bc.Tx{
					types.MapTx(&types.TxData{
						...
						SerializedSize: 1,
						Inputs:         []*types.TxInput{types.NewCoinbaseInput(nil)},
						Outputs:        []*types.TxOutput{types.NewTxOutput(*consensus.EYAssetID, 41250000000, cp)},
					}),
				},
			},
			parent: &state.BlockNode{
			...
			block: &bc.Block{
				...
				Transactions: []*bc.Tx{
					...
					types.MapTx(&types.TxData{
						Version:        1,
						SerializedSize: 1,
						Inputs:         []*types.TxInput{types.NewSpendInput([][]byte{}, *newHash(8), *consensus.EYAssetID, 100000000, 0, cp)},
						Outputs:        []*types.TxOutput{types.NewTxOutput(*consensus.EYAssetID, 90000000, cp)},
					}),
				},
			},
			parent: &state.BlockNode{
	...
	for i := 0; i < 100; i++ {
		block.Transactions = append(block.Transactions, types.MapTx(&types.TxData{
			...
			Inputs: []*types.TxInput{
				types.NewSpendInput([][]byte{}, *newHash(8), *consensus.EYAssetID, 10000000000, 0, cp),
			},
			Outputs: []*types.TxOutput{
				types.NewTxOutput(*consensus.EYAssetID, 9000000000, cp),
			},
		}))
	}
	...
	block := &bc.Block{
		...
		Transactions: []*bc.Tx{
			types.MapTx(&types.TxData{
				...
				SerializedSize: 1,
				Inputs:         []*types.TxInput{types.NewCoinbaseInput(nil)},
				Outputs:        []*types.TxOutput{types.NewTxOutput(*consensus.EYAssetID, 41449998224, cp)},
			}),
			types.MapTx(&types.TxData{
				Version:        1,
				SerializedSize: 1,
