...
func TestOverflow(t *testing.T) {
	...
	newTx := func(inputs []uint64, outputs []uint64) *bc.Tx {
		...
		for _, amount := range inputs {
			txInput := types.NewSpendInput(nil, *sourceID, *consensus.EYAssetID, amount, 0, ctrlProgram)
			txInputs = append(txInputs, txInput)
		}

		for _, amount := range outputs {
			txOutput := types.NewTxOutput(*consensus.EYAssetID, amount, ctrlProgram)
...
func TestTxValidation(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			f: func() {
				addCoinbase(consensus.EYAssetID, 100000, nil)
			},
			err: nil,
		},
		{
		...
		},
		{
			...
			f: func() {
				...
				addCoinbase(consensus.EYAssetID, 100000, arbitrary)
			},
...
func TestCoinbase(t *testing.T) {
	...
	CbTx := types.MapTx(&types.TxData{
		...
		Outputs: []*types.TxOutput{
			types.NewTxOutput(*consensus.EYAssetID, 888, cp),
		},
	})
			...
			block: &bc.Block{
				...
				Transactions: []*bc.Tx{
					...
					types.MapTx(&types.TxData{
						...
						Inputs: []*types.TxInput{
							types.NewCoinbaseInput(nil),
							types.NewSpendInput([][]byte{}, *newHash(8), *consensus.EYAssetID, 100000000, 0, cp),
						},
						Outputs: []*types.TxOutput{
							types.NewTxOutput(*consensus.EYAssetID, 888, cp),
							types.NewTxOutput(*consensus.EYAssetID, 90000000, cp),
						},
					}),
				},
			},
			...
			block: &bc.Block{
				...
				Transactions: []*bc.Tx{
					types.MapTx(&types.TxData{
						...
						Inputs: []*types.TxInput{
							types.NewCoinbaseInput(nil),
							types.NewSpendInput([][]byte{}, *newHash(8), *consensus.EYAssetID, 100000000, 0, retire),
						},
						Outputs: []*types.TxOutput{
							types.NewTxOutput(*consensus.EYAssetID, 888, cp),
							types.NewTxOutput(*consensus.EYAssetID, 90000000, cp),
...
func TestTimeRange(t *testing.T) {
	...
	tx := types.MapTx(&types.TxData{
		...
		Outputs: []*types.TxOutput{
			types.NewTxOutput(*consensus.EYAssetID, 1, []byte{0x6a}),
		},
	})
...
func mockGasTxInput() *types.TxInput {
	cp, _ := vmutil.DefaultCoinbaseProgram()
	return types.NewSpendInput([][]byte{}, *newHash(8), *consensus.EYAssetID, 100000000, 0, cp)
}
