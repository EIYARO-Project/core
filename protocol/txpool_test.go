...
var testTxs = []*types.Tx{
	...
	types.NewTx(types.TxData{
		...
		Inputs: []*types.TxInput{
			types.NewSpendInput(nil, bc.NewHash([32]byte{0x01}), *consensus.EYAssetID, 1, 1, []byte{0x51}),
		},
	...
	types.NewTx(types.TxData{
		...
		Outputs: []*types.TxOutput{
			types.NewTxOutput(*consensus.EYAssetID, 1, []byte{0x6b}),
		},
	}),
	//tx2
	types.NewTx(types.TxData{
	...
	types.NewTx(types.TxData{
		...
		Outputs: []*types.TxOutput{
			types.NewTxOutput(*consensus.EYAssetID, 0, []byte{0x51}),
		},
	}),
	//tx6
	types.NewTx(types.TxData{
		SerializedSize: 100,
		Inputs: []*types.TxInput{
			types.NewSpendInput(nil, bc.NewHash([32]byte{0x01}), *consensus.EYAssetID, 3, 1, []byte{0x51}),
			types.NewSpendInput(nil, testutil.MustDecodeHash("d84d0be0fd08e7341f2d127749bb0d0844d4560f53bd54861cee9981fd922cad"), bc.NewAssetID([32]byte{0xa1}), 3, 0, []byte{0x62}),
		},
		Outputs: []*types.TxOutput{
			types.NewTxOutput(*consensus.EYAssetID, 2, []byte{0x51}),
