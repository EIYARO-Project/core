...
func TestMapSpendTx(t *testing.T) {
	cases := []*TxData{
		&TxData{
			Inputs: []*TxInput{
				NewSpendInput(nil, testutil.MustDecodeHash("fad5195a0c8e3b590b86a3c0a95e7529565888508aecca96e9aeda633002f409"), *consensus.EYAssetID, 88, 3, []byte{1}),
			},
			Outputs: []*TxOutput{
				NewTxOutput(*consensus.EYAssetID, 80, []byte{1}),
			},
		},
		&TxData{
			Inputs: []*TxInput{
		...
		&TxData{
			Inputs: []*TxInput{
				NewIssuanceInput([]byte("nonce"), 254354, []byte("issuanceProgram"), [][]byte{[]byte("arguments1"), []byte("arguments2")}, []byte("assetDefinition")),
				NewSpendInput(nil, testutil.MustDecodeHash("db7b16ac737440d6e38559996ddabb207d7ce84fbd6f3bfd2525d234761dc863"), *consensus.EYAssetID, 88, 3, []byte{1}),
			},
			Outputs: []*TxOutput{
				NewTxOutput(*consensus.EYAssetID, 80, []byte{1}),
				NewTxOutput(*consensus.EYAssetID, 80, []byte{1}),
...
func TestMapCoinbaseTx(t *testing.T) {
	txData := &TxData{
		...
		Outputs: []*TxOutput{
			NewTxOutput(*consensus.EYAssetID, 800000000000, []byte{1}),
		},
	}
