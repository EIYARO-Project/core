...
func isTransactionNoEyInput(tx *types.Tx) bool {
...
func (tp *TxPool) IsDust(tx *types.Tx) bool {
	return isTransactionNoEyInput(tx) || isTransactionZeroOutput(tx)
}
...
func (tp *TxPool) addTransaction(txD *TxDesc) error {
	...
	for _, id := range tx.ResultIds {
		...
		if !txD.StatusFail || *output.Source.Value.AssetId == *consensus.EYAssetID {
			tp.utxo[*id] = tx
		}
	}
