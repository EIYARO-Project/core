...
func main() {
	...
	imports := map[string]bool{
		...
		"encoding/hex": true,
		"fmt":          true,
		"core/equity/compiler": true,
		"core/protocol/vm":     true,
	}
...
func asGoParams(params []*compiler.Param) (goParams string, imports []string) {
	...
	for _, p := range params {
		...
		case "Asset":
			typ = "bc.AssetID"
			imports = append(imports, "core/protocol/bc")
			strFlag = true
		case "Boolean":
			typ = "bool"
		case "Hash":
		...
		case "PublicKey":
			...
			imports = append(imports, "core/crypto/ed25519")
			strFlag = true
		case "Signature":
			typ = "[]byte"
			strFlag = true
	...
	if strFlag {
		imports = append(imports, "core/encoding/json")
	}
