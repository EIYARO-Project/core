	...
	cases := []struct {
	...
	}{
		{
			tx: types.NewTx(types.TxData{
				...
				Outputs: []*types.TxOutput{
					types.NewTxOutput(*consensus.EYAssetID, 41250000000, []byte{0x51}),
				},
			}),
			statusFail: false,
			wantUtxos:  []*account.UTXO{},
		},
		{
			tx: types.NewTx(types.TxData{
				...
				Outputs: []*types.TxOutput{
					types.NewTxOutput(*consensus.EYAssetID, 4125, []byte{0x51}),
				},
			}),
			statusFail: false,
			wantUtxos:  []*account.UTXO{},
		},
		{
			tx: types.NewTx(types.TxData{
				Inputs: []*types.TxInput{
					...
					types.NewSpendInput([][]byte{}, bc.Hash{V0: 3}, *consensus.EYAssetID, 5, 3, []byte{0x53}),
					types.NewSpendInput([][]byte{}, bc.Hash{V0: 4}, *consensus.EYAssetID, 7, 4, []byte{0x54}),
				},
				Outputs: []*types.TxOutput{
					types.NewTxOutput(bc.AssetID{V0: 1}, 4, []byte{0x51}),
					types.NewTxOutput(*consensus.EYAssetID, 12, []byte{0x53}),
				},
			}),
			statusFail: false,
			wantUtxos: []*account.UTXO{
		...
		},
		{
			tx: types.NewTx(types.TxData{
				Inputs: []*types.TxInput{
					...
					types.NewSpendInput([][]byte{}, bc.Hash{V0: 2}, *consensus.EYAssetID, 7, 1, []byte{0x51}),
				},
				Outputs: []*types.TxOutput{
					types.NewTxOutput(bc.AssetID{V0: 1}, 2, []byte{0x51}),
					types.NewTxOutput(bc.AssetID{V0: 1}, 3, []byte{0x52}),
					types.NewTxOutput(*consensus.EYAssetID, 2, []byte{0x53}),
					types.NewTxOutput(*consensus.EYAssetID, 5, []byte{0x54}),
				},
			}),
			statusFail:  false,
			vaildHeight: 0,
