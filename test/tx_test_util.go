...
func txFee(tx *types.Tx) uint64 {
	...
	for _, input := range tx.Inputs {
		if input.AssetID() == *consensus.EYAssetID {
			inputSum += input.Amount()
		}
	}

	for _, output := range tx.Outputs {
		if *output.AssetId == *consensus.EYAssetID {
...
func CreateCoinbaseTx(controlProgram []byte, height, txsFee uint64) (*types.Tx, error) {
	...
	if err := builder.AddOutput(types.NewTxOutput(*consensus.EYAssetID, coinbaseValue, controlProgram)); err != nil {
		return nil, err
	}
...
func CreateTxFromTx(baseTx *types.Tx, outputIndex uint64, outputAmount uint64, ctrlProgram []byte) (*types.Tx, error) {
	...
	output := types.NewTxOutput(*consensus.EYAssetID, outputAmount, ctrlProgram)
	builder := txbuilder.NewBuilder(time.Now())
