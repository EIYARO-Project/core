...
func getUPNPExternalAddress(externalPort, internalPort int) (*NetAddress, error) {
	...
	externalPort, err = nat.AddPortMapping("tcp", externalPort, internalPort, "eiyarod tcp", 0)
	if err != nil {
		return nil, errors.Wrap(err, "could not add tcp UPNP port mapping")
	}
	externalPort, err = nat.AddPortMapping("udp", externalPort, internalPort, "eiyarod udp", 0)
	if err != nil {
...
//DefaultListener Implements eiyarod server Listener
