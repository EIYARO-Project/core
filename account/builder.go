...
func (m *Manager) reserveEyUtxoChain(builder *txbuilder.TemplateBuilder, accountID string, amount uint64, useUnconfirmed bool) ([]*UTXO, error) {
	...
	for gasAmount := uint64(0); reservedAmount < gasAmount+amount; gasAmount = calcMergeGas(len(utxos)) {
		...
		res, err := m.utxoKeeper.Reserve(accountID, consensus.EYAssetID, reserveAmount, useUnconfirmed, builder.MaxTime())
		if err != nil {
			return nil, err
		}
...
func (m *Manager) buildEyTxChain(utxos []*UTXO, signer *signers.Signer) ([]*txbuilder.Template, *UTXO, error) {
	if len(utxos) == 0 {
	...
	for index := 0; index < len(utxos); index++ {
		...
		output := types.NewTxOutput(*consensus.EYAssetID, outAmount, acp.ControlProgram)
...
func SpendAccountChain(ctx context.Context, builder *txbuilder.TemplateBuilder, action txbuilder.Action) ([]*txbuilder.Template, error) {
	...
	if *act.AssetId != *consensus.EYAssetID {
		return nil, errors.New("spend chain action only support EY")
	}

	utxos, err := act.accounts.reserveEyUtxoChain(builder, act.AccountID, act.Amount, act.UseUnconfirmed)
	if err != nil {
		return nil, err
	}
	...
	tpls, utxo, err := act.accounts.buildEyTxChain(utxos, acct.Signer)
	if err != nil {
		return nil, err
	}
	...
	if utxo.Amount > act.Amount {
		if err = builder.AddOutput(types.NewTxOutput(*consensus.EYAssetID, utxo.Amount-act.Amount, utxo.ControlProgram)); err != nil {
