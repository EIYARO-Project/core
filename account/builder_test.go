...
func TestReserveEyUtxoChain(t *testing.T) {
	...
	for i, c := range cases {
		...
		utxos, err := m.reserveEyUtxoChain(&txbuilder.TemplateBuilder{}, "TestAccountID", c.amount, false)
...
func TestBuildEyTxChain(t *testing.T) {
	txbuilder.ChainTxUtxoNum = 3
	m := mockAccountManager(t)
	cases := []struct {
		inputUtxo  []uint64
	...
	for caseIndex, c := range cases {
		...
		tpls, gotUtxo, err := m.buildEyTxChain(utxos, acct.Signer)
		if err != nil {
			t.Fatal(err)
		}
...
func TestMergeSpendAction(t *testing.T) {
	testEY := &bc.AssetID{}
	if err := testEY.UnmarshalText([]byte("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")); err != nil {
		t.Fatal(err)
	}
