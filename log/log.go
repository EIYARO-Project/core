...
func newEyHook(logPath string) *EyHook {
	hook := &EyHook{lock: new(sync.Mutex)}
	hook.logPath = logPath
	return hook
}

// Write a log line to an io.Writer.
func (hook *EyHook) ioWrite(entry *logrus.Entry) error {
	module := "general"
...
func (hook *EyHook) Fire(entry *logrus.Entry) error {
	hook.lock.Lock()
...
func (hook *EyHook) Levels() []logrus.Level {
