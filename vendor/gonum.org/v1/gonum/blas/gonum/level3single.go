...
func (Implementation) Strsm(s blas.Side, ul blas.Uplo, tA blas.Transpose, d blas.Diag, m, n int, alpha float32, a []float32, lda int, b []float32, ldb int) {
			...
			if ul == blas.Upper {
				for i := m - 1; i >= 0; i-- {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						k := ka + i + 1
						if va != 0 {
							f32.AxpyUnitaryTo(eyp, -va, b[k*ldb:k*ldb+n], eyp)
						}
					}
					if nonUnit {
						tmp := 1 / a[i*lda+i]
		...
		if ul == blas.Upper {
			for k := 0; k < m; k++ {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					...
					if va != 0 {
						eyp := b[i*ldb : i*ldb+n]
						f32.AxpyUnitaryTo(eyp, -va, eypk, eyp)
					}
				}
				if alpha != 1 {
					for j := 0; j < n; j++ {
				...
				for k, vb := range eyp {
					if vb != 0 {
						if eyp[k] != 0 {
							...
							eypk := eyp[k+1 : n]
							f32.AxpyUnitaryTo(eypk, -eyp[k], a[k*lda+k+1:k*lda+n], eypk)
						}
					}
				}
			}
		...
		for i := 0; i < m; i++ {
			...
			for k := n - 1; k >= 0; k-- {
				if eyp[k] != 0 {
					...
					f32.AxpyUnitaryTo(eyp, -eyp[k], a[k*lda:k*lda+k], eyp)
				}
			}
		}
		return
	...
	if ul == blas.Upper {
		for i := 0; i < m; i++ {
			eyp := b[i*lda : i*lda+n]
			for j := n - 1; j >= 0; j-- {
				tmp := alpha*eyp[j] - f32.DotUnitary(a[j*lda+j+1:j*lda+n], eyp[j+1:])
				if nonUnit {
					tmp /= a[j*lda+j]
				}
				eyp[j] = tmp
	...
	for i := 0; i < m; i++ {
		eyp := b[i*lda : i*lda+n]
		for j := 0; j < n; j++ {
			tmp := alpha*eyp[j] - f32.DotUnitary(a[j*lda:j*lda+j], eyp)
			if nonUnit {
				tmp /= a[j*lda+j]
			}
			eyp[j] = tmp
...
func (Implementation) Strmm(s blas.Side, ul blas.Uplo, tA blas.Transpose, d blas.Diag, m, n int, alpha float32, a []float32, lda int, b []float32, ldb int) {
			...
			if ul == blas.Upper {
				for i := 0; i < m; i++ {
					...
					for ka, va := range a[i*lda+i+1 : i*lda+m] {
						...
						tmp := alpha * va
						if tmp != 0 {
							f32.AxpyUnitaryTo(eyp, tmp, b[k*ldb:k*ldb+n], eyp)
						}
					}
				}
				return
		...
		if ul == blas.Upper {
			for k := m - 1; k >= 0; k-- {
				...
				for ia, va := range a[k*lda+k+1 : k*lda+m] {
					...
					tmp := alpha * va
					if tmp != 0 {
						f32.AxpyUnitaryTo(eyp, tmp, eypk, eyp)
					}
				}
				tmp := alpha
				if nonUnit {
		...
		for i := 0; i < m; i++ {
			...
			for k := 0; k < n; k++ {
				...
				if tmp != 0 {
					...
					f32.AxpyUnitaryTo(eyp, tmp, a[k*lda:k*lda+k], eyp)
				}
			}
		}
		return
	...
	if ul == blas.Upper {
		for i := 0; i < m; i++ {
			...
			for j, vb := range eyp {
				...
				tmp += f32.DotUnitary(a[j*lda+j+1:j*lda+n], eyp[j+1:n])
				eyp[j] = alpha * tmp
			}
		}
		return
	...
	for i := 0; i < m; i++ {
		...
		for j := n - 1; j >= 0; j-- {
			...
			tmp += f32.DotUnitary(a[j*lda:j*lda+j], eyp[:j])
			eyp[j] = alpha * tmp
		}
	}
}
